...
public interface Parser<T> extends ReplyParser<T> {
    <U> U parse(ByteBuffer buffer, Visitor<? super T, U> visitor);

    @Override
    default <U> U parseReply(ByteBuffer buffer, ReplyVisitor<? super T, U> visitor) {
        return parse(buffer, visitor);
    }
